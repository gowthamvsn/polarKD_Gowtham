...
if uploaded:
    ...
    # --- Heuristic fallback: if LLM didn't mark anything primary, pick by mention count ---
    llm_picked_primary = any(r["_is_primary"] for r in rows)
    if not llm_picked_primary and rows:
    ...
    c3.metric("Secondary datasets", len(secondary_rows))
